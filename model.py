...
class PPNet(nn.Module):
    ...
    def forward(self, x, y):
        ...
        masked_distances = torch.mul(distances,y)
        min_masked_distances = -F.max_pool2d(-masked_distances,
                                      kernel_size=(masked_distances.size()[2],
                                                   masked_distances.size()[3]))
        min_masked_distances = min_masked_distances.view(-1, self.num_prototypes)
        masked_activation = self.distance_2_similarity(min_masked_distances)
        masked_logits = self.last_layer(masked_activation)
        total_activation = torch.sum(self.distance_2_similarity(distances),dim=(2,3))
        total_masked_activation = torch.sum(self.distance_2_similarity(masked_distances),dim=(2,3))
        activation = self.distance_2_similarity(distances)
        return logits, min_distances, masked_logits, min_masked_distances,activation,total_activation,total_masked_activation
